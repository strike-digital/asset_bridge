...
class SingletonUpdater:
    ...
    def check_interval(self):
        return (self._check_interval_enabled, self._check_interval_months, self._check_interval_days,
                self._check_interval_hours, self._check_interval_minutes)
    ...
    def get_tags(self):
        ...
        elif ((len(self._tags) - len(self._include_branch_list) == 0 and self._include_branches) or
              (len(self._tags) == 0 and not self._include_branches) and self._prefiltered_tag_count > 0):
            self._tag_latest = None
    ...
    def create_backup(self):
        ...
        if self._backup_ignore_patterns is not None:
            try:
                shutil.copytree(self._addon_root,
                                tempdest,
            ...
            for file in files:
                ...
                if os.path.isfile(dest_file):
                    ...
                    if replaced:
                        ...
                        self.print_verbose("Pattern not matched to {}, not overwritten".format(
    ...
    def check_for_update_async(self, callback=None):
        ...
        is_ready = (self._json is not None and "update_ready" in self._json and self._json["version_text"] != dict() and
                    self._json["update_ready"])
    ...
    def start_async_check_update(self, now=False, callback=None):
        ...
        check_thread = threading.Thread(target=self.async_check_update, args=(
            now,
            callback,
        ))
...
class BitbucketEngine:
    ...
    def get_zip_url(self, name, updater):
        return "https://bitbucket.org/{user}/{repo}/get/{name}.zip".format(user=updater.user,
                                                                           repo=updater.repo,
                                                                           name=name)
