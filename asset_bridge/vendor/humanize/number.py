...
def ordinal(value: NumberOrString, gender: str='male') -> str:
    ...
    try:
        ...
        return str(value)
    if gender == 'male':
        t = (P_('0 (male)', 'th'), P_('1 (male)', 'st'), P_('2 (male)', 'nd'), P_('3 (male)', 'rd'), P_('4 (male)', 'th'), P_('5 (male)', 'th'), P_('6 (male)', 'th'), P_('7 (male)', 'th'), P_('8 (male)', 'th'), P_('9 (male)', 'th'))
    else:
        t = (P_('0 (female)', 'th'), P_('1 (female)', 'st'), P_('2 (female)', 'nd'), P_('3 (female)', 'rd'), P_('4 (female)', 'th'), P_('5 (female)', 'th'), P_('6 (female)', 'th'), P_('7 (female)', 'th'), P_('8 (female)', 'th'), P_('9 (female)', 'th'))
...
def intcomma(value: NumberOrString, ndigits: int | None=None) -> str:
    ...
    while True:
        new = re.sub('^(-?\\d+)(\\d{3})', f'\\g<1>{thousands_sep}\\g<2>', orig)
        if orig == new:
            return new
        orig = new
powers = [10 ** x for x in (3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 100)]
human_powers = (NS_('thousand', 'thousand'), NS_('million', 'million'), NS_('billion', 'billion'), NS_('trillion', 'trillion'), NS_('quadrillion', 'quadrillion'), NS_('quintillion', 'quintillion'), NS_('sextillion', 'sextillion'), NS_('septillion', 'septillion'), NS_('octillion', 'octillion'), NS_('nonillion', 'nonillion'), NS_('decillion', 'decillion'), NS_('googol', 'googol'))
    ...
    for ordinal_, power in enumerate(powers[1:], 1):
        if value < power:
            ...
            if float(format % chopped) == powers_difference:
                ...
                return (negative_prefix + ' '.join([format, _ngettext(singular, plural, math.ceil(chopped))])) % chopped
...
def apnumber(value: NumberOrString) -> str:
    ...
    if not 0 <= value < 10:
        return str(value)
    return (_('zero'), _('one'), _('two'), _('three'), _('four'), _('five'), _('six'), _('seven'), _('eight'), _('nine'))[value]
...
def clamp(value: float, format: str='{:}', floor: float | None=None, ceil: float | None=None, floor_token: str='<', ceil_token: str='>') -> str:
    ...
    msg = 'Invalid format. Must be either a valid formatting string, or a function that accepts value and returns a string.'
